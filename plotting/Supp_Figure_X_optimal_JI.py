...
if __name__ == '__main__':
	...
	# 	cell_num_list = []
	# 	for JI in JI_list:
	# 		mask = pickle.load(bz2.BZ2File(f'{img_dir}/mask_{method}_matched_3D_final_{JI}.pkl', 'r'))
	# 		cell_num_list.append(len(np.unique(mask)-1))
	# 	print(cell_num_list)
	# 	np.save(f'{img_dir}/cell_num_JI.npy', cell_num_list)
	
	
	
		#
		# metrics_z_JI = np.stack(metrics_z_JI_pieces, axis=0)
		# for img_idx in metrics_z_JI.shape[1]:
		# 	metrics_z_JI_image = metrics_z_JI[:, img_idx, :]
		# 	metrics_z_JI_image_pca = pca.transform(metrics_z_JI_image)
		#
