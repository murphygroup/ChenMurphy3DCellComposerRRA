...
if __name__ == '__main__':
	# channel = sys.argv[1]
	# channel = 'actin'
	data_dir = f'/data/3D/IMC_3D/florida-3d-imc'
	...
	# methods = ['deepcell_membrane-0.12.6_0.1', 'deepcell_membrane-0.12.6_0.3', 'deepcell_membrane-0.12.6_0.5', 'cellstitch', 'aics_ml']
	...
	# methods = ['deepcell_membrane-0.12.6', 'cellpose-2.2.2', 'aics_ml']
	# methods_vis = ['DeepCell', 'Cellpose', 'AICS(ML)']
	# methods_vis = ['DeepCell', 'AICS(ML)']
	...
	# _, umap_model = pickle.load(open(join(pca_dir, 'umap_IMC_3D.pkl'), 'rb'))
	...
	for method in methods:
		...
		# avg_method_metrics_umap_pieces = list()
		...
		for noise in noise_list:
			...
			if method in methods_2D:
				...
				# print(optimal_JI)
				# metrics_dir_list = sorted(glob.glob(f'{data_dir}/**/{noise}/**/metrics_{method}_{optimal_JI}.npy', recursive=True))
			else:
			...
			# current_metrics_umap = umap_model.transform(current_metrics_z)
			print(current_metrics)
			
			# print(current_metrics_pca)

			avg_current_metrics_pca = np.average(current_metrics_pca, axis=0)
			# avg_current_metrics_umap = np.average(current_metrics_umap, axis=0)
			# print(avg_current_metrics_pca)

			avg_method_metrics_pca_pieces.append(avg_current_metrics_pca)
			# avg_method_metrics_umap_pieces.append(avg_current_metrics_umap)
			...
			if noise == 'original':
				...
				# print(noise)
				# print(np.average(current_metrics, axis=0))
			# if method == 'cellpose-2.2.2_2D-3D':
			# 	print(method)
			# 	print(noise)
			# 	print(np.average(current_metrics, axis=0))
		avg_method_metrics_pca = np.vstack(avg_method_metrics_pca_pieces)
		# avg_method_metrics_umap = np.vstack(avg_method_metrics_umap_pieces)
		...
		# plt.plot(avg_method_metrics_umap[:, 0], avg_method_metrics_umap[:, 1], color=cmap[method_idx])
		# plt.scatter(avg_method_metrics_umap[:, 0], avg_method_metrics_umap[:, 1], edgecolors=cmap[method_idx], s=50, facecolors='none', marker = marker[method_idx])
		# plt.scatter(avg_method_metrics_umap[0, 0], avg_method_metrics_umap[0, 1], edgecolors=cmap[method_idx], s=50, facecolors=cmap[method_idx], marker = marker[method_idx], label=methods_vis[method_idx])
		# print(avg_method_metrics_pieces)
	

	plt.xlabel(f'PC1({pc1_explained})')
	plt.ylabel(f'PC2({pc2_explained})')
	# plt.xlabel(f'UMAP Dimension 1')
	# plt.ylabel(f'UMAP Dimension 2')
	# plt.legend(loc='lower left')
	# plt.xlim([-4.5, 3.5])
	# plt.ylim([-1.5, 2.7])
	plt.savefig(f'{os.path.dirname(pca_dir)}/fig/IMC_PCA.png', dpi=500, bbox_inches='tight')
	# plt.savefig(f'{os.path.dirname(pca_dir)}/fig/IMC_UMAP.png', dpi=500)
	plt.clf()
