...
imsave(f'{IMAGEpath}/original/mask_aics_classic_matched_3D_final_0.2.tif', mask_2D_XY_stack)
np.load('./data/IMC_3D/florida-3d-imc/a296c763352828159f3adfa495becf3e/original/metrics/metrics_aics_classic_0.0.npy')
np.load('./data/IMC_3D/florida-3d-imc/a296c763352828159f3adfa495becf3e/original/metrics/metrics_aics_classic_0.2.npy')
...
def create_slice_fig_new(volume, slice_type, index=None, repaired_coords=None, fragment_idx=None):
	...
	# Function to reindex and convert to integer type.
	def reindex_and_convert(array):
		if len(np.unique(array)) > 2:
			unique_values = np.unique(array[array > 0])
			# Add mappings for non-zero values.
			reindex_map = {0: 0, fragment_idx: 20}
			# Start mapping other unique values from 2 onwards.
			reindex_map.update({value: i + 2 for i, value in enumerate(unique_values) if value != fragment_idx})
			# Use reindex_map.get(value, 0) to handle unexpected values gracefully.
	...
	base_colors = plt.cm.tab20.colors[2:]  # Adjusted to exclude similar colors if needed

	# Define a custom color mapping: white for 0, cyan for 1, then others
	custom_colors = ['white', 'cyan'] + list(base_colors)

	# Create a new colormap from the custom colors
...
def isolate_cell(segmentation, cell_index):
	...
	# return isolated_cell, np.array((int(np.mean(cell_coords[0])), int(np.mean(cell_coords[1])), int(np.mean(cell_coords[2]))))
