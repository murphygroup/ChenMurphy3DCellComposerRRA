	...
	for JI_threshold in JI_threshold_list:
		for img_dir in img_3D_dir_list:
			for method in methods:
				if True: 
						if not os.path.exists(f'{img_dir}/mask_{method}_matched_3D_{JI_threshold}.pkl'):
						#if True:
							print(f'{img_dir}/mask_{method}_matched_3D_{JI_threshold}.pkl')
							#os.system(f'srun -p short1,pool1,model1,model2,model3,model4,pool3-bigmem,gpu -t 72:00:00 -n 1 -c 4 --mem 20G python /home/haoranch/projects/HuBMAP/2D-3D/script/match_3D_cells.py {img_dir} {method} {JI_threshold} &')
				# 		break
				# 	break
				# break
