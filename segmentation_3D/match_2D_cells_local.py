...
if __name__ == '__main__':
	...
	# method = sys.argv[2]
	# axis = sys.argv[3]
	# JI_thre = float(sys.argv[4])
	#
	# data_dir = '/data/3D/IMC_3D/florida-3d-imc/a296c763352828159f3adfa495becf3e/original'
	method = 'deepcell_membrane-0.12.6'
	# axis = 'XY'
	JI_thre = 0.0
	
	# data_dir_list = glob.glob('/data/3D/IMC_3D/florida-3d-imc/**/original', recursive=True)
	data_dir_list = glob.glob('/data/3D/IMC_3D/**/original', recursive=True)
	for data_dir in data_dir_list[1:]:
		...
		for axis in ['XY','YZ','XZ']:
			...
			imsave(f'{data_dir}/mask_{method}_matched_stack_{axis}_{JI_thre}.tif', new_img.astype(np.int16) + np.min(new_img.astype(np.int16)))
			...
			# pickle.dump(new_img_coords, bz2.BZ2File(f'{data_dir}/mask_{method}_matched_stack_coords_' + axis + '_0.3.pkl'), 'wb'))
		
		
		
			# Plot histogram
			# plt.hist(best_JI_list, bins=[i/20.0 for i in range(21)], edgecolor="k", alpha=0.7)
			#
			# plt.xlabel("Value Range")
			# plt.ylabel("Frequency")
			# plt.title("Histogram of best_JI_list values")
			# plt.show()
			# plt.clf()
