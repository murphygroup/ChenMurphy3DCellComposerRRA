...
if __name__ == '__main__':
	...
	method = sys.argv[2]
	try:
		nuclear_slice = np.load(join(file_dir, 'nuclear_mask_' + method + '_XY.npy'))
	except:
		nuclear_slice = np.load(join(file_dir, 'nuclear_mask_artificial_XY.npy'))
	
	cell_mask_3D = pickle.load(bz2.BZ2File(join(file_dir, 'mask_' + method + '_matched_3D_no_bits.pickle'), 'r'))
	
	repaired_nuclear_slices = []
	matched_cell_slices = []
	for slice_idx in range(nuclear_slice.shape[0]):
		matched_cell_slice, repaired_nuclear_slice = match_repair_cell_nucleus(nuclear_slice[slice_idx], cell_mask_3D[slice_idx])
	...
	# matched_cell_membrane_3D = get_membrane_mask(matched_cell_3D)
	# matched_nuclear_membrane_3D = get_membrane_mask(matched_nuclear_3D)
	
	# matched_cell_membrane_3D = get_indexed_mask(matched_cell_3D, find_boundaries(matched_cell_3D, mode='inner'))
	# matched_nuclear_membrane_3D = get_indexed_mask(matched_nuclear_3D, find_boundaries(matched_nuclear_3D, mode='inner'))
	matched_cell_outside_nucleus_3D = matched_cell_3D - matched_nuclear_3D
	pickle.dump(matched_cell_3D, bz2.BZ2File(join(file_dir, method + '_cell_matched_3D_filtered.pickle'), 'w'))
	pickle.dump(matched_nuclear_3D, bz2.BZ2File(join(file_dir, method + '_nuclear_matched_3D_filtered.pickle'), 'w'))
	pickle.dump(matched_cell_outside_nucleus_3D, bz2.BZ2File(join(file_dir, method + '_cell_outside_nucleus_matched_3D_filtered.pickle'), 'w'))
	# imsave(join(file_dir, method + '_cell_matched_3D.pickle'), matched_cell_3D)
	# imsave(join(file_dir, method + '_nuclear_matched_3D.pickle'), matched_nuclear_3D)
	# imsave(join(file_dir, method + '_cell_outside_nucleus_matched_3D.tif'), matched_cell_outside_nucleus_3D)

	# image = np.stack((matched_cell_3D, matched_nuclear_3D, matched_cell_outside_nucleus_3D))
	# channel_names = ['cells', 'nuclei', 'cell_outside_nucleus']
	# OmeTiffWriter.save(image, join(file_dir, method + '_3_channels.ome.tiff'), channel_names=channel_names)
	#
	#
	# matched_cell_3D = pickle.load(bz2.BZ2File(join(file_dir, method + '_cell_matched_3D.pickle'), 'r'))
	# matched_nuclear_3D = pickle.load(bz2.BZ2File(join(file_dir, method + '_nuclear_matched_3D.pickle'), 'r'))
	# matched_cell_outside_nucleus_3D = pickle.load(bz2.BZ2File(join(file_dir, method + '_cell_outside_nucleus_matched_3D.pickle'), 'r'))
	#
