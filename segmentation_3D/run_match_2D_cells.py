	...
	for JI_threshold in JI_threshold_list:
		for img_dir in img_3D_dir_list:
			for method in methods:
				for axis in axes:
						if not os.path.exists(f'{img_dir}/mask_{method}_matched_stack_{axis}_{JI_threshold}.pkl'):
						#if True:
							print(f'{img_dir}/mask_{method}_matched_stack_{axis}_{JI_threshold}.pkl')
							os.system(f'srun -p short1,pool1,model1,model2,model3,model4,pool3-bigmem,gpu -t 72:00:00 -n 1 -c 4 --mem 20G python /home/haoranch/projects/HuBMAP/2D-3D/script/match_2D_cells_local.py {img_dir} {method} {axis} {JI_threshold} &')
				# 		break
				# 	break
				# break
